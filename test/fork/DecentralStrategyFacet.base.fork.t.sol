...
interface IDecentralStrategyFacet {
    ...
    function finalizeDecentralYieldWithdrawal(uint256 index) external returns (uint256 received);

    function requestDecentralPrincipalWithdrawal(uint256 index) external;

    function finalizeDecentralPrincipalWithdrawal(uint256 index) external returns (uint256 received);

    // -------------------------
    // Views
    // -------------------------
    function decentralPositions() external view returns (NFTPosition[] memory);
...
contract DecentralStrategyFacetBaseForkTest is Test {
    ...
    address constant DECENTRAL_POOL = 0x6fC42888f157A772968CaB5B95A4e42a38C07fD0;
    ...
    function test_fork_decentralDeposit_USDC_success() external {
        ...
        uint256 amount = minAmt > 0 ? minAmt : 100 * 10 ** USDC_DECIMALS;
        ...
        console2.log("user USDC balance after deal:", IERC20(BASE_USDC).balanceOf(user));
        ...
        console2.log("vault balanceOf(user, PROJECT_ID):", asyncVault.balanceOf(user, PROJECT_ID));
        console2.log("vault USDC balance:", IERC20(BASE_USDC).balanceOf(address(asyncVault)));

        IDecentralStrategyFacet.NFTPosition[] memory positions = facet.decentralPositions();
    ...
    function test_fork_principal_request_finalization_success() external {
        ...
        positions = facet.decentralPositions();
        console2.log("principalRequested flag after request:", positions[0].principalRequested);
        ...
        console2.log("position.principalRequested:", positions[0].principalRequested);

        assertFalse(positions[0].principalRequested);
        // closed may be true or false depending on partial vs full redemption

        console2.log("vault USDC balance after finalize:", IERC20(BASE_USDC).balanceOf(address(asyncVault)));

        console2.log("vault async shares still escrowed:", asyncVault.balanceOf(address(asyncVault), PROJECT_ID));

        assertEq(asyncVault.balanceOf(address(asyncVault), PROJECT_ID), shares);
        ...
        uint256 userBalanceBefore = IERC20(BASE_USDC).balanceOf(user);

        console2.log("user USDC balance before fulfill:", userBalanceBefore);
        ...
        assertEq(asyncVault.balanceOf(address(asyncVault), PROJECT_ID), 0);
    ...
    function test_fork_yield_request_finalization_success() external {
        ...
        positions = facet.decentralPositions();
        console2.log("yieldRequested flag after request:", positions[0].yieldRequested);

        assertTrue(positions[0].yieldRequested);

        (uint256 withdrawalAmount, uint256 requestTimestamp, bool exists, bool approved) =
        ...
        (,, bool existsAfter, bool approvedAfter) = pool.getYieldWithdrawalRequest(tokenId);
        ...
        console2.log("position.closed:", positions[0].closed);
        console2.log("position.yieldRequested:", positions[0].yieldRequested);
