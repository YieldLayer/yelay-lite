...
async function main() {
    ...
    const token = ERC20__factory.connect(underlyingAsset, user);
    const decimals = await token.decimals();
    const amount = ethers.parseUnits(String(AMOUNT), decimals);
    const allowance = await token.allowance(user.address, contracts.yelayLiteVault);
    if (allowance === 0n) {
        await token.approve(contracts.yelayLiteVault, ethers.MaxUint256).then((tx) => tx.wait(10));
    }
    try {
